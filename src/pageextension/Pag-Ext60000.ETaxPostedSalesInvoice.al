pageextension 60000 "ETax Posted Sales Invoice" extends "Posted Sales Invoice"
{
        ...
        addafter("Update Document")
        {
            action("Postman")
            {
                ...
                var
                    ...
                    tblGeneralLedgerSetup: Record "General Ledger Setup";
                    count: Integer;
                    NetIncTaxTotalMount: Decimal;
                    TaxBasisTotalMount: Decimal;
                    TaxTotalMount: Decimal;
                begin
                    tblAPISetup.Get();
                    tblGeneralLedgerSetup.Get();
                    tblAPIBody.Init();
                    ...
                    tblAPIBody."F03-INVOICE_CURRENCY_CODE" := tblGeneralLedgerSetup."LCY Code";
                    tblAPIBody."F04-TAX_TYPE_CODE1" := 'VAT';
                    tblAPIBody."F05-TAX_CAL_RATE1" := format(Rec."VAT Base Discount %");
                    tblAPIBody."F06-BASIS_AMOUNT1" := '0';
                    tblAPIBody."F07-BASIS_CURRENCY_CODE1" := tblGeneralLedgerSetup."LCY Code";
                    tblAPIBody."F08-TAX_CAL_AMOUNT1" := '0';
                    tblAPIBody."F09-TAX_CAL_CURRENCY_CODE1" := tblGeneralLedgerSetup."LCY Code";
                    tblAPIBody."F36-ORIGINAL_TOTAL_AMOUNT" := '0';
                    tblAPIBody."F37-ORIGINAL_TOTAL_CURRENCY_CODE" := tblGeneralLedgerSetup."LCY Code";
                    tblAPIBody."F38-LINE_TOTAL_AMOUNT" := '0';
                    tblAPIBody."F39-LINE_TOTAL_CURRENCY_CODE" := tblGeneralLedgerSetup."LCY Code";
                    tblAPIBody."F46-TAX_BASIS_TOTAL_AMOUNT" := '0';
                    tblAPIBody."F47-TAX_BASIS_TOTAL_CURRENCY_CODE" := tblGeneralLedgerSetup."LCY Code";
                    tblAPIBody."F48-TAX_TOTAL_AMOUNT" := '0';
                    tblAPIBody."F49-TAX_TOTAL_CURRENCY_CODE" := tblGeneralLedgerSetup."LCY Code";
                    tblAPIBody."F50-GRAND_TOTAL_AMOUNT" := '0';
                    tblAPIBody."F51-GRAND_TOTAL_CURRENCY_CODE" := tblGeneralLedgerSetup."LCY Code";
                    ...
                    if tblSaleInvoiceLine.FindSet() then
                        repeat
                            ...
                            tblAPILine."L11-PRODUCT_CHARGE_CURRENCY_CODE" := tblGeneralLedgerSetup."LCY Code";
                            ...
                            tblAPILine."L23-LINE_BASIS_CURRENCY_CODE" := tblGeneralLedgerSetup."LCY Code";
                            tblAPILine."L24-LINE_TAX_CAL_AMOUNT" := format(round(tblSaleInvoiceLine."Amount Including VAT", 0.01) - round(tblSaleInvoiceLine.Amount, 0.01)).Replace(',', '');
                            tblAPILine."L25-LINE_TAX_CAL_CURRENCY_CODE" := tblGeneralLedgerSetup."LCY Code";
                            tblAPILine."L33-LINE_NET_TOTAL_AMOUNT" := '0';
                            tblAPILine."L34-LINE_NET_TOTAL_CURRENCY_CODE" := tblGeneralLedgerSetup."LCY Code";
                            tblAPILine."L35-LINE_NET_INCLUDE_TAX_TOTAL_AMOUNT" := format(round(tblSaleInvoiceLine."Amount Including VAT", 0.01)).Replace(',', '');
                            tblAPILine."L36-LINE_NET_INCLUDE_TAX_TOTAL_CURRENCY_CODE" := tblGeneralLedgerSetup."LCY Code";
                            ...
                            TaxBasisTotalMount := TaxBasisTotalMount + round(tblSaleInvoiceLine.Amount, 0.01);
                            TaxTotalMount := TaxTotalMount + (round(tblSaleInvoiceLine."Amount Including VAT", 0.01) - round(tblSaleInvoiceLine.Amount, 0.01));
                    ...
                    tblAPIBody."F46-TAX_BASIS_TOTAL_AMOUNT" := Format(TaxBasisTotalMount).Replace(',', '');
                    tblAPIBody."F48-TAX_TOTAL_AMOUNT" := Format(TaxTotalMount).Replace(',', '');
    ...
    procedure initResponse(tblAPIBody: Record "API Body")
    var
        tblAPIResponse: Record "API Response";
        tblAPISetup: Record "API Setup";
    begin
